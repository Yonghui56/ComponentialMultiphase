	...
	virtual void assembleODE(const NumLib::TimeStep & /*time*/, const MeshLib::IElement &e, const LocalVectorType & u1, const LocalVectorType & u0, LocalMatrixType & localM, LocalMatrixType & localK, LocalVectorType & localF)
	{
		...
		*/
		...
		for (j = 0; j < n_gsp; j++)
		{
			...
			* Store the value
			*/
			_function_data->get_deriv_Sg_dX()->setIntegrationPointValue(ele_id, j, dSgdX_gp);
			_function_data->get_deriv_Sg_dP()->setIntegrationPointValue(ele_id, j, dSgdP_gp);
			//+++++++++++++++++++++++++End Calculation++++++++++++++++++++++++++++++++++++++++++++++
			/*
			if (S_gp(0) <= 0.0){
				Charact_func = 0;
			...
			*/
			...
			M(0, 0) = 0.0;
			M(0, 1) = poro;
			M(1, 0) = -poro*(rho_l_std)*dSgdP_gp(0);
			M(1, 1) = poro*(1 - dSgdX_gp(0)*rho_l_std);
			//-------------debugging------------------------
			//std::cout << "M=" << std::endl;
			//std::cout << M << std::endl;
			//--------------end debugging-------------------
			...
			D(0, 0) = poro*(1 - S_gp(0))*D_L*C_h*Omega_gp(0)*rho_L_std / RHO_L + Lambda_h + rho_G_h_gp(0)*lambda_G*dSgdP_gp(0)*dPC_dSg_gp(0);
						
			D(0, 1) = poro*(1 - S_gp(0))*D_L*(C_h*InterM_gp(0))*rho_L_std / RHO_L + rho_G_h_gp(0)*lambda_G*dPC_dSg_gp(0)*dSgdX_gp(0);
						
			D(1, 0) = RHO_L*lambda_L + RHO_G*lambda_G + \
					RHO_G*lambda_G*dPC_dSg_gp(0, 0)*dSgdP_gp(0);
			D(1, 1) = RHO_G*lambda_G*dPC_dSg_gp(0, 0)*dSgdX_gp(0);
			//-------------debugging------------------------
			//std::cout << "D=" << std::endl;
			//std::cout << D << std::endl;
			//--------------end debugging-------------------
