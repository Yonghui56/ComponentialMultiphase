...
#ifndef SINGLE_STEP_CMP_HETERO_PCPGFORM_H 
#define SINGLE_STEP_CMP_HETERO_PCPGFORM_H 
...
>
class SingleStepCMP_hetero_PCPGForm
...
public:
	...
	*/
	SingleStepCMP_hetero_PCPGForm(MyDiscreteSystem*      dis,
	...
	*/
	virtual ~SingleStepCMP_hetero_PCPGForm()
	{
		/*
...
private:
	DISALLOW_COPY_AND_ASSIGN(SingleStepCMP_hetero_PCPGForm);
...
template <
	...
	class T_USER_NON_LINEAR_SOLUTION
>
SingleStepCMP_hetero_PCPGForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::SingleStepCMP_hetero_PCPGForm(MyDiscreteSystem*                dis,
                           UserFemProblem*                  problem,
						   UserNonLinearSolution*           nlin_solution,
						   UserFunctionData*                function_data)
...
_problem(problem), _nlin_solution(nlin_solution), _discrete_system(dis), _function_data(function_data)
{
	...
	INFO("->Setting up a solution algorithm for CMP_hetero_PCPGForm problem.");
	...
	for (i_var = 0; i_var < n_var; i_var++) {
		...
		for (i = 0; i < var->getNumberOfDirichletBC(); i++) {
			...
			for (j = 0; j < list_bc_nodes.size(); j++)
			{
				...
				// set BC values
				if (i_var == 0)
					_function_data->set_PG_node_values(node_id, node_value);
				else if (i_var == 1)
...
template <class T_USER_FUNCTION_DATA,
		  ...
		  class T_USER_NON_LINEAR_PROBLEM,
		  class T_USER_NON_LINEAR_SOLUTION>
int SingleStepCMP_hetero_PCPGForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::solveTimeStep(const NumLib::TimeStep &t_n1)
{
	// print solving information
	INFO("--Solving nonlinear equation for the CMP_hetero_PCPGForm process. \n");
	
	_nlin_solution->solveTimeStep(t_n1);

    // update the P and X vector
	_function_data->set_PG(_nlin_solution->getCurrentSolution(0));
