...
*
*
* \file FunctionCMP_hetero_PCPGForm.hpp
*
...
bool FunctionCMP_hetero_PCPGForm<T1, T2>::initialize(const BaseLib::Options &option)
{
	...
	_problem = new MyCMPPCPGFormProblemType(dis);
	_problem->setTimeSteppingFunction(*tim);  // applying the time stepping function
	
	// creating GAS pressure vector
	MyVariableCMPPCPGForm* PG = _problem->addVariable("GAS_PRESSURE");
	FemVariableBuilder var_builder;
	var_builder.doit("GAS_PRESSURE", option, msh, femData->geo, femData->geo_unique_name, _feObjects, PG);
	...
	MyVariableCMPPCPGForm* PC = _problem->addVariable("CAPILLARY_PRESSURE");
	var_builder.doit("CAPILLARY_PRESSURE", option, msh, femData->geo, femData->geo_unique_name, _feObjects, PC);
	...
	*/
	...
	_non_linear_problem = new MyNonLinearCMPPCPGFORMProblemType(dis);
	...
	SolutionLib::FemIC* PG_ic = new SolutionLib::FemIC(msh);
	PG_ic->addDistribution(femData->geo->getDomainObj(), new NumLib::TXFunctionDirect<double>(_PG->getDiscreteData()));
	...
	_non_linear_problem->getVariable(0)->setIC(PG_ic);
	...
	// set up solution
	_solution = new MyCMPPCPGFormSolution(dis, _problem, _non_linear_solution, this);
	/**
...
template <class T1, class T2>
void FunctionCMP_hetero_PCPGForm<T1, T2>::initializeTimeStep(const NumLib::TimeStep &/*time*/)
{
	/*
	size_t i;
...
template <class T1, class T2>
void FunctionCMP_hetero_PCPGForm<T1, T2>::updateOutputParameter(const NumLib::TimeStep &/*time*/)
{

}

template <class T1, class T2>
void FunctionCMP_hetero_PCPGForm<T1, T2>::output(const NumLib::TimeStep &/*time*/)
{
...
template <class T1, class T2>
void FunctionCMP_hetero_PCPGForm<T1, T2>::calc_nodal_eos_sys(double dt = 0.0)
{
