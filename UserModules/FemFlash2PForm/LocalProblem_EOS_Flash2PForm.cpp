...
/**
  ...
  */
LocalProblem_EOS_Flash2PForm::LocalProblem_EOS_Flash2PForm()
{
...
/**
  ...
  */
LocalProblem_EOS_Flash2PForm::~LocalProblem_EOS_Flash2PForm(void)
{
 ...
 */
void LocalProblem_EOS_Flash2PForm::solve(ogsChem::LocalVector & Input, ogsChem::LocalVector & Output)
{
...
/**
  ...
  */
void LocalProblem_EOS_Flash2PForm::solve_minimization(ogsChem::LocalMatrix & J,
...
/**
  ...
  */
void LocalProblem_EOS_Flash2PForm::solve_LocalProblem_Newton_LineSearch(std::size_t & flag)
{
	...
	*/
	...
	MathLib::LocalVector x = MathLib::LocalVector::Zero(N);
	MathLib::LocalVector y = MathLib::LocalVector::Zero(N);
	//start flash calculation
	MathLib::LocalVector Z = MathLib::LocalVector::Zero(8);//8*1 vector total molar fraction for each component
	MathLib::LocalVector K = MathLib::LocalVector::Zero(8);//8*1 vector K-value for each component

	double sum(0.0);
	double vf(0.0);
	//start the newton iteration
	for (int ii = 0; ii < Iter_tol; ii++){
		if (ii >= Iter_tol)
		{
			WARN("Solving Peng Robinson EOS failed!!!!");
			break;
		}
		for (int nn = 0; nn < N - 1; ++nn){
			Z(nn) = U_pre(nn + 1);
			sum += Z(nn);
			K(nn) = _EOS->func_wilson(U_pre(0), 403.15, P_crit(nn), T_crit(nn), acentric(nn));
		}
		Z(7) = 1 - sum;
		vf = _EOS->solve_Rachford_Rice(vf, Z, K);
		//calc xi yi
		for (int nn = 0; nn < N - 1; ++nn){
			x(nn) = Z(nn) / (1 + (K(nn) - 1)*vf);
			y(nn) = K(nn)*x(nn);
		}
