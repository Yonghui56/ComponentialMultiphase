...
#ifndef NON_LINEAR_CMP_FLASH2PFORM_TIME_ODE_LOCAL_ASSEMBLER_H
#define NON_LINEAR_CMP_FLASH2PFORM_TIME_ODE_LOCAL_ASSEMBLER_H
...
class NonLinearCMP_Flash2PForm_TimeODELocalAssembler : public T
{
public:
	typedef MathLib::LocalVector LocalVectorType;
	typedef MathLib::LocalMatrix LocalMatrixType;
	NonLinearCMP_Flash2PForm_TimeODELocalAssembler(FemLib::LagrangeFeObjectContainer* feObjects, T_FUNCTION_DATA* func_data, const MeshLib::CoordinateSystem &problem_coordinates)
	...
	virtual ~NonLinearCMP_Flash2PForm_TimeODELocalAssembler() {
		_function_data = NULL;
...
protected:
	...
	virtual void assembleODE(const NumLib::TimeStep & /*time*/, const MeshLib::IElement &e, const LocalVectorType & u1, const LocalVectorType & u0, LocalMatrixType & localM, LocalMatrixType & localK, LocalVectorType & localF)
	{
		...
		*/
		/*
		*/
		Input = LocalVectorType::Zero(5); //five primary variables
		...
		*/
		...
		for (j = 0; j < n_gsp; j++)
		{
			...
			X1_gp = Np*u1.block(n_nodes, 0, n_nodes, 1);
			X2_gp = Np*u1.block(2*n_nodes, 0, n_nodes, 1);
			X3_gp = Np*u1.block(3*n_nodes, 0, n_nodes, 1);
			X4_gp = Np*u1.tail(n_nodes);
			...
			D(0, 0) = lambda_G*X1_gp(0, 0)*mrho_G_gp(0) + lambda_L*X1_gp(0, 0)*mrho_L_gp(0);
			D(1, 0) = lambda_G*X2_gp(0, 0)*mrho_G_gp(0) + lambda_L*X2_gp(0, 0)*mrho_L_gp(0);
			D(2, 0) = lambda_G*X3_gp(0, 0)*mrho_G_gp(0) + lambda_L*X3_gp(0, 0)*mrho_L_gp(0);
			D(3, 0) = lambda_G*X4_gp(0, 0)*mrho_G_gp(0) + lambda_L*X4_gp(0, 0)*mrho_L_gp(0);
			D(4, 0) = lambda_G*X5_gp(0,0)*mrho_G_gp(0) + lambda_L*X5_gp(0, 0)*mrho_L_gp(0);
			//-------------debugging------------------------
			//std::cout << "D=" << std::endl;
			//std::cout << D << std::endl;
			//--------------end debugging-------------------
...
private:
	...
	LocalVectorType mrho_L_gp;//molar density
	LocalVectorType mrho_G_gp;//
	LocalVectorType rho_G_gp;
	LocalVectorType rho_L_gp;
	LocalVectorType dPGh_dPG_gp;
	LocalVectorType dPC_dSg_gp;
	LocalVectorType PG_gp;
	LocalVectorType PL_gp;
	LocalVectorType S_gp;
	LocalVectorType X5_gp;
